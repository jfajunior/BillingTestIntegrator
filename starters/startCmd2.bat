...
::-----------------------------------------------------------------------------
C:\P9911106\Programas\jdk1.7.0_79\bin\java -version:1.7 -version > nul 2>&1
...
FOR /F "skip=2 tokens=2*" %%a IN ('REG QUERY "HKLM\Software\JavaSoft\Java Development Kit\1.7" /v JavaHome 2^>nul') DO ( 
	set JavaPath=%jdk_path%bin\java
)
if "[%JavaPath%]" == "[]" (
...
echo Chamando: %JavaPath% -jar %~dp0..\build\BillingTestIntegrator.jar
echo. 
echo. 
"%JavaPath%" -jar %~dp0..\build\BillingTestIntegrator.jar
